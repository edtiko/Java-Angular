        ...
        try {
            ...
            if (Objects.equals(userDto.getRoleId(), RoleEnum.ATLETA.getId())) {
                ...
                if (trainingPlanUser != null) {
                    ...
                    UserFitting fittingActive = userFittingService.findByUser(userDto.getUserId());
                    if (fittingActive != null) {
                        userSession.setUserFittingId(fittingActive.getUserFittingId());
                    }
                }
            ...
            if (jsonResponse != null && !jsonResponse.isEmpty()) {
                ...
                if (statusRes.equals("success")) {
                    ...
                    if (jo.get("planId") != null && !jo.get("planId").isJsonNull()
                        ...
                        Integer productId = jo.get("productId").getAsInt();
                        if (productId == 350) { //ID PRODUCTO = 350 FITTING EN WORDPRESS
                            UserFitting userFitting = userFittingService.findByUser(userDto.getUserId());

                            if (userFitting == null) {
                                userFitting = new UserFitting();
                                userFitting.setUserId(new User(userDto.getUserId()));
                                userFitting.setOrderId(userTrainingOrder.getOrderId());
                                userFitting.setOrderItemId(userTrainingOrder.getOrderItemId());
                                userFitting.setCreationDate(Calendar.getInstance().getTime());
                                userFitting.setStateId(StateEnum.ACTIVE.getId().shortValue());
                                userFittingService.create(userFitting);
                            }
