trainingApp.controller('AsesoresController', ['$scope', 'AsesorService', '$window', '$location', '$mdDialog',
    function ($scope, AsesorService, $window, $location, $mdDialog) {
        var self = this;
        $scope.userSession = JSON.parse($window.sessionStorage.getItem("userInfo"));
        $scope.asesores = [];
        $scope.asesoresFiltered = [];
        ...
        self.getAsesores = function () {
            AsesorService.getAsesores($scope.userSession.userId).then(
                    function (data) {
                        $scope.asesores = data.output;
                        $scope.asesoresFiltered = data.output;
                    },
                    function (error) {
                        console.log(error);
                    }
